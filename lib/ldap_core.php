...
class LDAP {
	...
	function LogError() {
		if ($this->connection_handle !== false) {
			...
			$this->error_no = 'ldap connection failed';
		}
	}
	...
	function GetLDAPInfo ($type) {
		...
		if ($type == LDAP_INDEX_PHONE) 			return empty($this->info[0][$phone]) ? '' : $this->info[0][$phone];
		if ($type == LDAP_INDEX_HOMEDIR) 		return $this->info[0][$homedir][0];
		if ($type == LDAP_INDEX_MEMBER) 		return empty($this->info[0][$member]) ? '' : $this->info[0][$member];
